...
RULES: dict[str, Rule] = {
    ...
    "persistence_item_modified": _rule(
        "persistence_item_modified",
        "Persistence Launch Item Modified",
        "persistence",
        "An existing LaunchAgent or LaunchDaemon definition changed after the previous daemon snapshot.",
        "high",
        "high",
        "persistence_monitor",
        ["approved software update", "administrator configuration change", "managed service repair"],
        ["Compare the plist content hash and executable target with the prior snapshot.", "Review the target signature, owner, package receipt, and nearby installer activity."],
        "T1543.001",
    ),
    "persistence_item_removed": _rule(
        "persistence_item_removed",
        "Persistence Launch Item Removed",
        "persistence",
        "An existing LaunchAgent or LaunchDaemon disappeared after the previous daemon snapshot.",
        "medium",
        "high",
        "persistence_monitor",
        ["approved uninstall", "administrator cleanup", "managed software replacement"],
        ["Confirm that the removal was authorized.", "Review nearby process, installer, and integrity events for defense-evasion activity."],
        "T1543.001",
    ),
    "persistence_artifact_added": _rule(
        "persistence_artifact_added",
        "Persistence Artifact Added",
        "persistence",
        "A privileged helper, extension, cron entry, startup item, or Background Task Management artifact appeared.",
        "high",
        "high",
        "persistence_artifact_detector",
        ["approved application install", "system or security software update", "managed endpoint deployment"],
        ["Inspect ownership, permissions, signature, package receipt, and executable target.", "Confirm the responsible installer or administrator action."],
        "T1547",
    ),
    "persistence_artifact_modified": _rule(
        "persistence_artifact_modified",
        "Persistence Artifact Modified",
        "persistence",
        "A monitored persistence artifact changed content or filesystem identity.",
        "high",
        "high",
        "persistence_artifact_detector",
        ["approved application update", "managed policy refresh", "expected Background Task Management update"],
        ["Compare the artifact fingerprint with the prior snapshot.", "Review signature, owner, responsible process, and surrounding timeline before remediation."],
        "T1547",
    ),
    "persistence_artifact_removed": _rule(
        "persistence_artifact_removed",
        "Persistence Artifact Removed",
        "persistence",
        "A monitored persistence artifact disappeared after the previous daemon snapshot.",
        "medium",
        "high",
        "persistence_artifact_detector",
        ["approved uninstall", "administrator cleanup", "managed software replacement"],
        ["Confirm the removal was authorized.", "Review integrity and process events for attempted defense evasion."],
        "T1547",
    ),
    ...
    "trusted_usb_device_connected": _rule(
        "trusted_usb_device_connected",
        "Trusted USB Device Connected",
        "hardware",
        "A user-trusted USB device reconnected.",
        "info",
        "high",
        "hardware_detector",
        ["approved accessory reconnect", "dock reconnect"],
        ["Confirm the trusted USB device reconnect was expected."],
    ),
    "untrusted_usb_device_connected": _rule(
        "untrusted_usb_device_connected",
        "Untrusted USB Device Connected",
        "hardware",
        "A user-untrusted USB device connected.",
        "high",
        "high",
        "hardware_detector",
        ["previously marked test device", "quarantined accessory"],
        ["Disconnect or isolate the device until it is reviewed."],
    ),
    "usb_device_reconnected": _rule(
        "usb_device_reconnected",
        "USB Device Reconnected",
        "hardware",
        "A known USB device reconnected.",
        "low",
        "high",
        "hardware_detector",
        ["expected reconnect", "hub topology refresh"],
        ["Confirm the reconnect was expected."],
    ),
    "usb_device_changed": _rule(
        "usb_device_changed",
        "USB Device Changed",
        "hardware",
        "A known USB device changed port or identity properties.",
        "medium",
        "medium",
        "hardware_detector",
        ["dock movement", "firmware descriptor update"],
        ["Review device identity and location changes."],
    ),
    "usb_storage_device_connected": _rule(
        "usb_storage_device_connected",
        "USB Storage Device Connected",
        "hardware",
        "A USB storage-class device connected.",
        "high",
        "high",
        "hardware_detector",
        ["approved removable media", "backup drive"],
        ["Confirm removable storage is approved before opening files."],
    ),
    "usb_hid_device_connected": _rule("usb_hid_device_connected", "USB HID Device Connected", "hardware", "A USB human-interface device connected.", "high", "high", "hardware_detector", ["approved keyboard, mouse, or controller"], ["Confirm the input device is expected."]),
    "usb_keyboard_connected": _rule("usb_keyboard_connected", "USB Keyboard Connected", "hardware", "A USB keyboard connected.", "high", "high", "hardware_detector", ["approved keyboard"], ["Confirm the keyboard is expected."]),
    "usb_mouse_connected": _rule("usb_mouse_connected", "USB Mouse Connected", "hardware", "A USB mouse connected.", "high", "high", "hardware_detector", ["approved mouse"], ["Confirm the mouse is expected."]),
    "usb_trackpad_connected": _rule("usb_trackpad_connected", "USB Trackpad Connected", "hardware", "A USB trackpad connected.", "high", "high", "hardware_detector", ["approved trackpad"], ["Confirm the trackpad is expected."]),
    "usb_network_adapter_connected": _rule("usb_network_adapter_connected", "USB Network Adapter Connected", "hardware", "A USB network adapter connected.", "high", "high", "hardware_detector", ["approved Ethernet or Wi-Fi adapter"], ["Confirm the network adapter is expected and review network changes."]),
    "usb_camera_connected": _rule("usb_camera_connected", "USB Camera Connected", "hardware", "A USB camera connected.", "high", "medium", "hardware_detector", ["approved webcam"], ["Confirm the camera is expected."]),
    "usb_microphone_connected": _rule("usb_microphone_connected", "USB Microphone Connected", "hardware", "A USB microphone connected.", "high", "medium", "hardware_detector", ["approved microphone or headset"], ["Confirm the microphone is expected."]),
    "usb_unknown_class_connected": _rule("usb_unknown_class_connected", "Unknown USB Class Connected", "hardware", "A USB device of unknown class connected.", "high", "medium", "hardware_detector", ["incomplete descriptor", "hub passthrough"], ["Review vendor, product, and serial identity."]),
    "physical_device_connected": _rule("physical_device_connected", "Physical Device Connected", "hardware", "A physical device connected.", "medium", "medium", "hardware_detector", ["expected peripheral"], ["Review connected device identity."]),
    "physical_device_removed": _rule("physical_device_removed", "Physical Device Removed", "hardware", "A physical device was removed.", "low", "medium", "hardware_detector", ["expected disconnect"], ["Confirm removal was expected."]),
    ...
    "system_moisture_detected": _rule(
        ...
        "Liquid or Moisture Warning Observed",
        "hardware",
        "An affirmative liquid or moisture warning marker was observed in macOS hardware telemetry. The marker indicates a hardware-safety risk but does not by itself prove permanent damage.",
        "critical",
        "high",
        "hardware_detector",
        ["transient hardware warning", "diagnostic or service test", "hardware log artifact"],
        [
            "If safe, disconnect external power and accessories, stop charging, and shut down the Mac.",
            "Confirm whether liquid exposure occurred and note the affected port or area.",
            "Arrange Apple or authorized-service inspection before reconnecting power when exposure is plausible.",
            "Preserve the event and related hardware logs as a damage-history record.",
        ],
    ),
    ...
    "microphone_activity_confirmed": _rule(
        "microphone_activity_confirmed",
        "Microphone Activity Confirmed",
        "privacy",
        "A microphone device-state signal reported that audio capture started.",
        "high",
        "high",
        "native_event_helper",
        ["legitimate call", "voice recording", "accessibility or dictation session"],
        ["Confirm microphone use is expected.", "Review the attributed process and nearby lock, idle, camera, and remote-access events."],
    ),
    "microphone_activity_stopped": _rule(
        "microphone_activity_stopped",
        "Microphone Activity Stopped",
        "privacy",
        "A microphone device-state signal reported that audio capture stopped.",
        "info",
        "high",
        "native_event_helper",
        ["call ended", "recording stopped", "browser tab closed"],
        ["Match this transition to its start event.", "Confirm the attributed process ended capture as expected."],
    ),
    "capture_device_connected": _rule(
        "capture_device_connected",
        "Capture Device Connected",
        "privacy",
        "A camera or microphone device became available to the system.",
        "high",
        "high",
        "native_event_helper",
        ["approved webcam", "headset", "virtual conferencing device"],
        ["Confirm the device is authorized.", "Review whether an unfamiliar external or virtual device appeared."],
    ),
    "capture_device_disconnected": _rule(
        "capture_device_disconnected",
        "Capture Device Disconnected",
        "privacy",
        "A camera or microphone device was removed from the system.",
        "info",
        "high",
        "native_event_helper",
        ["device unplugged", "virtual camera application quit"],
        ["Confirm the removal was expected.", "Review nearby capture start and stop events."],
    ),
    ...
    "possible_keylogger_detected": _rule(
        "possible_keylogger_detected",
        "Possible Keylogger Detected",
        "malware",
        "An enabled keyboard event tap was correlated with process trust or scope indicators that require immediate review.",
        "high",
        "high",
        "keylogger_detection_scan",
        ["approved accessibility utility", "password manager", "remote-support software", "keyboard remapping tool"],
        ["Verify the process path and publisher.", "Review Input Monitoring and Accessibility permissions.", "Correlate the process with persistence and network activity before remediation."],
    ),
    "dylib_hijack_detected": _rule(
        "dylib_hijack_detected",
        "Possible Dynamic Library Hijack",
        "rootkit",
        "A running Mach-O executable has a suspicious rpath-shadowing or weak-library condition.",
        "high",
        "high",
        "rootkit_dylib_hijack_scanner",
        ["development build", "plug-in host", "compatibility framework", "intentionally disabled library validation"],
        ["Preserve both files and their hashes.", "Verify code signatures, Team IDs, entitlements, and dyld search order.", "Correlate the library with process, persistence, quarantine, and network evidence."],
    ),
    ...
    "user_notifier_integrity_mismatch": _rule(
        "user_notifier_integrity_mismatch",
        "User Notifier Integrity Mismatch",
        "integrity",
        "The user notifier plist, runtime path, permissions, or tracked files changed from the trusted manifest.",
        "high",
        "high",
        "integrity_check",
        ["authorized notifier repair", "expected application update", "package reinstall"],
        [
            "Compare the observed notifier plist, ProgramArguments, owner, mode, and hashes with the stored manifest.",
            "Review alert pipeline diagnostics for missed or redirected notifications.",
            "Reinstall the user notifier from a trusted copy if the change was not approved.",
        ],
        "T1562",
    ),
    "integrity_verified": _rule(
        "integrity_verified",
        "Application Integrity Verified",
        "integrity",
        "MSAA application files matched the trusted manifest.",
        "info",
        "high",
        "integrity_check",
    ...
    "integrity_modified": _rule(
        "integrity_modified",
        "Application Integrity Modified",
        "integrity",
        "MSAA application files changed from the trusted manifest.",
        "high",
        "high",
        "integrity_check",
        ["authorized update", "local development", "package repair"],
        ["View detailed differences.", "Export evidence before repair.", "Reinstall from a trusted source if the change was not approved."],
        "T1562",
    ),
    "integrity_missing_files": _rule(
        "integrity_missing_files",
        "Application Integrity Missing Files",
        "integrity",
        "Required MSAA application files are missing from the installation.",
        "critical",
        "high",
        "integrity_check",
        ["failed update", "incomplete copy", "authorized repair"],
        ["Repair missing components from a trusted source or reinstall MSAA."],
        "T1562",
    ),
    "integrity_extra_files": _rule(
        "integrity_extra_files",
        "Application Integrity Extra Files",
        "integrity",
        "Unexpected files were found in the MSAA application tree.",
        "high",
        "medium",
        "integrity_check",
        ["local development artifact", "packaging artifact"],
        ["Review unexpected files, especially executables, before trusting this installation."],
    ),
    "integrity_stale_manifest": _rule(
        "integrity_stale_manifest",
        "Application Integrity Stale Manifest",
        "integrity",
        "Trusted manifest metadata differs from the current MSAA build.",
        "medium",
        "medium",
        "integrity_check",
        ["trusted application update", "manifest cache drift"],
        ["Verify update provenance before creating a new trusted baseline."],
    ),
    ...
    "rootkit_suspect_detected": _rule(
        "rootkit_suspect_detected",
        "Rootkit-Like Suspect Indicators",
        "system_integrity",
        "Advanced persistence, visibility, extension, or integrity indicators were correlated for analyst review.",
        "critical",
        "medium",
        "rootkit_detection",
        ["security tooling", "VPN or driver install", "developer/debug posture", "tooling visibility race"],
        ["Preserve evidence.", "Review Rootkit & Advanced Persistence details.", "Correlate persistence, extension, network, and integrity timelines."],
        "T1014",
    ),
    "hidden_port_mismatch_detected": _rule(
        "hidden_port_mismatch_detected",
        "Hidden Port Visibility Mismatch",
        "network",
        "A local listener was visible in one source but lacked consistent owner visibility.",
        "high",
        "medium",
        "rootkit_detection",
        ["short-lived listener", "permission limitation", "parser race", "developer server"],
        ["Re-run local port checks close together in time.", "Review lsof, netstat, and local probe evidence."],
        "T1014",
    ),
    "suspicious_kernel_extension_detected": _rule(
        "suspicious_kernel_extension_detected",
        "Suspicious Kernel Extension",
        "persistence",
        "A kernel extension has attributes that require advanced persistence review.",
        "high",
        "medium",
        "rootkit_detection",
        ["hardware driver", "security product", "virtualization software", "MDM-approved extension"],
        ["Verify Team ID, signature, path, owner, permissions, and approval source."],
        "T1547.006",
    ),
    "suspicious_system_extension_detected": _rule(
        "suspicious_system_extension_detected",
        "Suspicious System Extension",
        "persistence",
        "A system, network, DriverKit, or Endpoint Security extension has review flags.",
        "high",
        "medium",
        "rootkit_detection",
        ["VPN client", "network filter", "EDR", "device driver", "MDM-approved extension"],
        ["Verify Team ID, signature, notarization, path, owner, permissions, and user/MDM approval."],
        "T1547.006",
    ),
    "system_integrity_weakened": _rule(
        "system_integrity_weakened",
        "System Integrity Weakened",
        "system_integrity",
        "A macOS integrity protection appears disabled or weakened.",
        "high",
        "high",
        "rootkit_detection",
        ["lab system", "developer debugging", "repair workflow", "intentionally reduced security"],
        ["Confirm whether the posture was intentional.", "Review SIP, authenticated root, Gatekeeper, FileVault, and boot args."],
        "T1562",
    ),
    "persistence_network_correlation_detected": _rule(
        "persistence_network_correlation_detected",
        "Persistence and Network Correlation",
        "persistence",
        "Persistence and local network indicators were correlated for high-priority review.",
        "critical",
        "medium",
        "rootkit_detection",
        ["approved daemon with listener", "developer service", "security product"],
        ["Inspect plist/program arguments, listener owner, binary hash, signature, and recent admin activity."],
        "T1543",
    ),
    "visibility_mismatch_detected": _rule(
        "visibility_mismatch_detected",
        "Visibility Mismatch Detected",
        "system_integrity",
        "Local visibility sources disagree on a component.",
        "high",
        "medium",
        "rootkit_detection",
        ["permission limit", "race condition", "process exited", "tool parsing issue"],
        ["Repeat collection close together in time and preserve raw outputs."],
        "T1014",
    ),
...
EVENT_TYPE_ALIASES: dict[str, str] = {
    ...
    "new_network_connection": "new_network_connection_detected",
    "new_listening_port": "new_listener_detected",
    "dns_changed": "new_dns_server_detected",
    "gateway_changed": "new_gateway_detected",
    "vpn_changed": "vpn_disconnected",
    "proxy_changed": "proxy_enabled",
    "network_visibility_mismatch": "localhost_visibility_mismatch_detected",
    ...
    "physical_device_removed": "physical_device_removed",
    "trusted_usb_device_connected": "trusted_usb_device_connected",
    "untrusted_usb_device_connected": "untrusted_usb_device_connected",
    "usb_storage_device_connected": "usb_storage_device_connected",
    "usb_hid_device_connected": "usb_hid_device_connected",
    "usb_keyboard_connected": "usb_keyboard_connected",
    "usb_mouse_connected": "usb_mouse_connected",
    "usb_trackpad_connected": "usb_trackpad_connected",
    "usb_network_adapter_connected": "usb_network_adapter_connected",
    "usb_camera_connected": "usb_camera_connected",
    "usb_microphone_connected": "usb_microphone_connected",
    "usb_unknown_class_connected": "usb_unknown_class_connected",
    "usb_inventory_changed": "usb_inventory_changed",
    "current_usb_device_inventory_changed": "usb_device_connected",
    "usb_reconnect_detected": "usb_device_connected",
    "usb_device_reconnected": "usb_device_reconnected",
    "bluetooth_inventory_changed": "bluetooth_inventory_changed",
