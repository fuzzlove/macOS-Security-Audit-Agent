...
from PySide6.QtWidgets import QApplication, QAbstractButton, QComboBox, QLineEdit, QTabWidget, QWidget
...
def audit_widget_controls(root: QWidget, *, section: str = "") -> list[dict[str, Any]]:
...
def run_ui_control_audit(context: AuditContext) -> list[FunctionalCheck]:
    ...
    try:
        ...
        if app is None:
            ...
            path = write_ui_control_audit(records)
...
def write_ui_control_audit(records: list[dict[str, Any]], path: Path | None = None) -> Path:
    path = path or Path("docs") / "PRE_UAT_UI_CONTROL_AUDIT.md"
